...
contract AT_X is AngelToken {
  ...
  function crowdfund_execution(address _owner, uint256 _id) public returns(bool executed){
    ...
    if(last_rnd_id != 0){
      ...
      //payout mapping
      uint256 _total_angels_share = (la_cost * la_num_to_issue) - 5500;// subtract estimated expenses & overhead to import, produce, and ship product
      for(uint256 i=1; i<=Angel_List.length; i++) {
        address angel = Angel_List[i];
      ...
      //seed round
      map_angel_to_payout[_id][_owner] = _total_angels_share * 2;
      //one transfer for seed round, can be done on layer 1
      /* _owner.transfer(la_cost * la_num_to_issue); */
    }
    last_rnd_id = _id;
    executed = true;
  }
